...
- 账号导出支持 access-token-only 账号；`refresh_token`、`id_token` 等缺失字段会导出为空字符串。
...
- 导出账号数据。仅有 `access_token` 的账号也可导出，缺失字段会保留为空字符串。
...
- 账号导出：支持 JSON 和 ZIP。仅有 `access_token` 的账号也可导出，缺失字段会导出为空字符串。
- 用户 API Key 管理。
...
`format` 可取 `json` 或 `zip`。`access_tokens` 为空数组时导出全部账号。仅有 `access_token` 的账号也会导出，缺失字段输出为空字符串。
